...
def users_ilike(term):
    ...
    table = Users.query.order_by(Users.name).filter((Users.name.ilike(term)) | (Users.grade.ilike(term)) | (Users.tags.ilike(term)))
...
def user_by_email(tags):
    """finds User in table matching email """
    return Users.query.filter_by(tags=tags).first()
